...
select 
    cp.user_id,
    (coalesce(cp.total, 0) - coalesce(cb.total, 0)) as remaining_credit
from "CREDIT_PURCHASE" cp
inner join "COURSE_BOOKING" cb
...
-- select 
-- 	sum(purchased_credits) as 用戶王小明的購買堂數
-- from "CREDIT_PURCHASE"
-- where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io');

-- select 
-- 	count(*) as 用戶王小明的已使用堂數
-- from "COURSE_BOOKING"
-- where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
-- and cancelled_at isnull;
...
-- 	sum(purchased_credits) as 用戶王小明的購買堂數
-- from "CREDIT_PURCHASE"
-- where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')) - (select 
-- 	count(*) as 用戶王小明的已使用堂數
-- from "COURSE_BOOKING"
-- where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
-- and cancelled_at isnull)
...
-- from "CREDIT_PURCHASE"
-- where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')) as "CREDIT_PURCHASE" 
-- inner join (select 
-- 	count(*)
-- from "COURSE_BOOKING"
-- where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
-- and cancelled_at isnull) as "COURSE_BOOKING"
-- on "COURSE_BOOKING".user_id = "CREDIT_PURCHASE".user_id;
...
select 
	(select name from "USER" where id = "COACH".user_id) as 教練名稱,
